...
'*****************************************************************
'***  TEMods
'***  04/30/2006  Changed Variable names and types.
'***
'*****************************************************************

Private arylngCRC32Table(255) As Long
...
'*****************************************************************
Public Function CRC32(sMessage As String) As Currency
    ...
    lngCRC = &HFFFFFFFF
    For lngIndex = 1 To Len(sMessage)
        bytChar = Asc(Mid(sMessage, lngIndex, 1))
        bytTemp = (lngCRC And &HFF) Xor bytChar
        lngA = ulShiftRightBy8(lngCRC)
        lngCRC = lngA Xor arylngCRC32Table(bytTemp)
    Next
    
    CRC32 = lngCRC Xor &HFFFFFFFF

End Function

Public Function ulShiftRightBy8(lngValue As Long) As Long

    ' Shift 32-bit long value to right by 8 bits
    ' Avoiding problem with sign bit
    Dim lngNew As Long
    
    lngNew = (lngValue And &H7FFFFFFF) \ 256
    If (lngValue And &H80000000) <> 0 Then
        lngNew = lngNew Or &H800000
    End If
    
    ulShiftRightBy8 = lngNew
...
Public Function CRC32Setup()
    ...
    For lngIndex = lngOffset To lngOffset + lngNumLength - 1
        arylngCRC32Table(lngIndex) = varArray(lngIndex - lngOffset)
    Next
    lngOffset = lngOffset + lngNumLength
...
End Function
'*** Declared as private because it is only needed during testing.
Private Sub TestCRC32()
...
'CRC32() = E96CCF45
    ...
    sMessage = "123456789"
    iCRC = CRC32(sMessage)
    MsgBox "CRC32(" & sMessage & ")=" & Hex(iCRC)
